    ...
    <>
      <div className="text-center md:text-left">
        <button
          ...
          className="rounded-full border-2 border-cyan-50 px-4 py-2 text-slate-900 hover:bg-cyan-50 focus:outline-none focus:ring-2 focus:ring-cyan-300 focus:ring-offset-2"
        >
      ...
      <Transition.Root show={isOpen} as={Fragment}>
        <Dialog onClose={setIsOpen} className="fixed inset-0 z-10 overflow-y-auto px-4 pt-[20vh]">
          <Transition.Child
            ...
            leaveTo="opacity-0"
          >
            <Dialog.Overlay className="fixed inset-0 bg-cyan-900/80" />
          </Transition.Child>
          <Transition.Child
            enter="transition ease-out duration-300"
              ...
              <Combobox.Options static className="max-h-96 overflow-y-auto border-t py-4 text-sm">
                {filteredTimezones.map((item) => (
                  <Combobox.Option key={item} value={item}>
                    {({ active }) => (
                      <div className={cx('px-8 py-2', active ? 'bg-cyan-500' : 'bg-white')}>
                        <div className="flex gap-2">
                          ...
                          <span
                            className={cx(
                              active ? 'text-cyan-200' : 'font-semibold text-slate-400'
                            )}
                          >
